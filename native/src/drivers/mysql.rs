...
impl Driver for MysqlDriver {
    ...
    async fn connect(&self, spec: &ConnSpec, net: NetContext) -> anyhow::Result<Box<dyn Connection>> {
        ...
        if !tls.wanted() {
            ...
            return Ok(Box::new(MysqlConnection { conn, opts, encrypted: false }));
        }
        // Encrypted attempt.
        let opts = build_opts(spec, &net, true).await?;
        match Conn::new(opts.clone()).await {
            Ok(conn) => Ok(Box::new(MysqlConnection { conn, opts, encrypted: true })),
            Err(e) => {
                if tls.required() {
                    Err(anyhow::anyhow!(
                    ...
                    let conn = Conn::new(opts.clone())
                        .await
                        .map_err(|e| anyhow::anyhow!("mysql connect failed: {e}"))?;
                    Ok(Box::new(MysqlConnection { conn, opts, encrypted: false }))
                }
            }
        }
    }
